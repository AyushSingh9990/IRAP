...
function RegistrySearch({
    ...
    return values
      .filter((entry, index, collection) =>
        collection.findIndex(
          (candidate) =>
            candidate.name === entry.name && candidate.stateCode === entry.stateCode,
        ) === index,
      )
  ...
  const submit = (event) => {
    ...
    navigate(`/directory${params.size ? `?${params.toString()}` : ''}`);
  };

  return (
    <form className={styles.searchPanel} onSubmit={submit} aria-label="Registry search">
      <div className={styles.grid}>
        ...
        <FormField label="City">
          ...
          >
            ...
            {cities.map((entry) => (
              <option key={`${entry.name}-${entry.stateCode}`} value={entry.name}>
                {entry.name}{entry.stateCode ? `, ${entry.stateCode}` : ''}
              </option>
            ))}
          </Select>
        </FormField>
      ...
      <Button className={styles.submit} icon="search" type="submit">
        Search the registry
      </Button>
    </form>
  );
